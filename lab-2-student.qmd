...
```{r}
#| label: load-data
survey_data <- read_csv('data/surveys.csv', show_col_types = FALSE)
spec(survey_data) 
dim(survey_data)
```
...
7 columns are double, 7 are character, and 1 is a date
...
ggplot(data = survey_data, 
  ...
  geom_point(alpha = .1) + # alpha value of .1 makes groups easier to see
  facet_wrap(vars(species)) +
  labs(title = "Rodent Hindfoot Length in mm by Weight and Species",
       x = "Weight (g)",
       y = "")
...
species_mod <- aov(weight ~ species, data = survey_data)
...
As the p value is less than the significance value of 0.001, we can reject the null hypothesis and conclude that there are statistically significant differences between the different species' mean weight. 
